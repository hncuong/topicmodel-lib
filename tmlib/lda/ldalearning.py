...
class LdaLearning(object):
	...
	def static_online(self):
		raise NotImplementedError( "Should have implemented this" )

	def learn_model(self, formatted_data, batch_size=5000,
	                suffle=False,term_ passes=1,
	                save_model_every=0, compute_sparsity_every=0,
	                save_statistic=False, save_top_words_every=0,
	                num_top_words=20, vocab_file='', model_folder='model'):
	mini_batch_no = 0
	...
	for pass_no in range(passes):
		...
		while True:
			wordids_tks, cts_lens = formatted_data.load_minibatch_term_freq(datafp, batch_size)
			if len(wordids_tks) == 0:
				break
			mini_batch_no += 1
			logger.info("\t\tMini batch no: %s", mini_batch_no)
			time_e, time_m, theta = static_online(wordids_tks, cts_lens)
