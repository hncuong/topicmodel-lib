...
If you've read `Online OPE`_ and `Streaming OPE`_, you can see that they are hybrid algorithms which combine OPE inference with variational Bayes for estimating the posterior of the global variables. They have to maintain variational parameters (:math:`\lambda`) for the
...
-------
  ...
  ::
    ...
    model = streaming_ope.learn_model(save_model_every=1, compute_sparsity_every=1, save_statistic=True, save_top_words_every=1, num_top_words=10, model_folder='models-ml-ope')
...
.. [2] Khoat Than, Tung Doan, “Guaranteed inference in topic models". [Online]. Available at: https://arxiv.org/abs/1512.03308
